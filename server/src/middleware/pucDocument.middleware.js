...
const UPLOAD_ROOT = path.resolve(__dirname, "../../uploads/puc");
...
const uploadPucDocument = (req, res, next) => {
    upload.single("document")(req, res, (error) => {
        if (error instanceof multer.MulterError) {
            const message = error.code === "LIMIT_FILE_SIZE"
                ? "PUC document must be 5 MB or smaller"
                : "Invalid PUC document upload";
...
const getDocumentReference = (file, userId) => file
    ? path.posix.join("puc", String(userId), file.filename)
    : undefined;
